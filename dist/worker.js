...
app.post("/api/admin/import", jwtAuth, async (c) => {
  ...
  for (const record of records) {
    ...
    imported.push(result.meta.last_row_id);
  }
...
app.get("/api/admin/export", jwtAuth, async (c) => {
  ...
  return new Response(csv, {
    headers: {
      ...
      "Content-Disposition": `attachment; filename="inventory_${status}_${(/* @__PURE__ */ new Date()).toISOString().split("T")[0]}.csv"`
    }
  });
});
